...
- **Fixes unbounded memory growth when the upload endpoint is unreachable.** The recorder could not survive its own transport failing: a failed batch was re-queued while new events kept arriving, and *each failure was reported through `FlutterError.reportError`* — which, with `captureFlutterErrors` on (the default), the recorder converts straight back into a recorded event. That event then triggered the flush that was failing. Every failure therefore manufactured more of the work that was failing, forever. Observed in a host app whose endpoint had no DNS record: ~63 events/sec generated while completely idle, Dart heap climbing ~34MB/min, 492MB RSS after 10 minutes, ~32k retained events. Four independent defects, all fixed:
  - `_sessionHistory` was appended to for every event and trimmed only when the session restarted, so a long-running app retained every event (each holding its message and stack-trace strings) for the whole process. Now capped by `maxSessionHistoryEvents` (default 1000), oldest dropped first. It is only read by `buildReplayDocument()`, so nothing else is affected.
  - The unsent event buffer had no ceiling — `maxBatchSize` only *triggers* a flush, it never bounded the list. Now capped by `maxBufferedEvents` (default 500). The pending *snapshot* backlog already had a byte ceiling; the event buffer did not.
  - No circuit breaker: an unreachable endpoint was retried indefinitely. After `maxConsecutiveTransportFailures` (default 5) consecutive failures the recorder now suspends uploads **and capture**, drops the undeliverable snapshot backlog, and backs off (`transportFailureBackoff`, default 30s, doubling to `maxTransportFailureBackoff`, default 15m). One probe is allowed when the backoff expires; a success closes the breaker and resumes capture.
  - Routine transport failures are no longer reported as framework errors on every occurrence — only the first of each streak, marked `silent`, with a re-entrancy guard so the report cannot be turned back into an event. `flush()` also no longer rethrows: it is called through `unawaited(...)`, so rethrowing surfaced as an *unhandled async error*, which a host wiring `capturePlatformErrors` turns into yet another event — a second amplification path for the same failure.
- **New config:** `maxBufferedEvents`, `maxSessionHistoryEvents`, `maxConsecutiveTransportFailures`, `transportFailureBackoff`, `maxTransportFailureBackoff`. All have defaults; no host changes are required. Set a cap to `0` to disable it (not recommended).
- No breaking changes. Hosts get the bounds automatically on upgrade.
