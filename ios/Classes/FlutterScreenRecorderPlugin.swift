...
public class FlutterScreenRecorderPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
  private var captureManager = IOSReplayCaptureManager()

  public static func register(with registrar: FlutterPluginRegistrar) {
    let instance = FlutterScreenRecorderPlugin()
    let methodChannel = FlutterMethodChannel(
      name: "flutter_screen_recorder/methods",
      binaryMessenger: registrar.messenger()
    )
    let eventChannel = FlutterEventChannel(
      name: "flutter_screen_recorder/events",
      binaryMessenger: registrar.messenger()
    )
